...
APPSTORAGENAME="`echo "$STORAGEBASENAME" | sed -e 's/-//g' | sed -E 's/^(.*)$/\L\1/g' | head -c 20`out"
...
cd 
git init
git remote add azure "https://$DEPLOYMENTUSERNAME:$DEPLOYMENTPASSWORD@$WEBAPPNAME.scm.azurewebsites.net/$WEBAPPNAME.git"
git add -A
git commit 
cd $PWD
